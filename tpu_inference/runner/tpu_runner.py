...
class TPUModelRunner(KVConnectorModelRunnerMixin, LoRAModelRunnerMixin):
    ...
    def _execute_single_step_decode(
        ...
        scheduler patch (commit b8e7bdaf) fixes this by advancing
        ``num_computed_tokens`` correctly for fused-decode outputs.  The
        custom wiring was replaced by a continue_decode delegation
        (commit 1f798d2c) as a diagnostic, but that ALSO garbled because
        ``_precompile_continue_decode`` does not run when
        ``enable_continue_decode=False`` — so the delegated continue_decode
