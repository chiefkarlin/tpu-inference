...
class CompilationManager:
    ...
    def _precompile_single_step_decode(self) -> None:
        """Precompile the single_step_decode fused dispatch.

        Mirrors ``_precompile_continue_decode`` but builds the simpler
        inputs needed by ``single_step_decode`` (no while_loop, no
        TpuSamplingState, no max_decode_steps — but adds logits_indices
        and input_positions as explicit traced args).
        """
        logger.info("Precompiling single_step_decode fused dispatch.")
        dp_size = self.runner.vllm_config.sharding_config.total_dp_size
        dp_sharding = NamedSharding(
            self.runner.mesh, PartitionSpec(ShardingAxisName.ATTN_DATA, ))
        ...
        for num_reqs in self.runner.num_reqs_paddings:
            ...
            def single_step_warmup(_fn, _args, _call_kwargs):
                new_args = list(_args)
                # arg index 5 = kv_caches (positional in wrapper signature)
                new_args[5] = self.runner.kv_caches
                return _fn(*new_args, **_call_kwargs)

            self._run_compilation(
                f"worker{self.runner.rank} "
                f"single_step_decode_reqs_{num_reqs}",
                single_step_wrapper,
                self.runner.state_leaves,
                getattr(self.runner.model, "step_fn_no_options",
                        self.runner.model_fn),
